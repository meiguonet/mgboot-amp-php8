...
final class Response
{
    ...
    private Request $req;
    private AmpResponse|null $ampResponse = null;
    ...
    private function __construct(Request $req, ?AmpResponse $response = null)
    {
        $this->req = $req;

        if ($response instanceof AmpResponse) {
            $this->ampResponse = $response;
        }
    }

    public static function create(Request $req, ?AmpResponse $response = null): self
    {
        return new self($req, $response);
    }
    ...
    public function sendByFpm(): void
    {
    ...
    public function sendByAmpResponse(): AmpResponse
    {
        /**
         * @var AmpResponse $resp
         */
        $resp = $this->ampResponse;
        ...
        if ($statusCode >= 400) {
            $reason = $this->getErrorReason($statusCode);
            $resp->setStatus($statusCode, empty($reason) ? null : $reason);
            $resp->setBody('');
            return $resp;
        }
        ...
        if ($isAttachment) {
            list($attachmentFileName, $fromFile, $contentLength, $contents) = $this->handleAttachmentResponseForAmp($contents);

            if (!is_string($contents) || empty($contents)) {
                $this->payload = HttpError::create(400);
                return $this->sendByAmpResponse();
            }
        ...
        if ($isImage) {
            list($fromFile, $contents) = $this->handleImageResponseForAmp($contents);
        ...
        if (($isAttachment || $isImage) && $fromFile) {
            ...
            $stream = new ResourceInputStream(fopen($contents, 'r'));
            $resp->setBody($stream);
            return $resp;
        }
    ...
    private function preSend(): array
    {
        ...
        if ($this->ampResponse instanceof AmpResponse) {
            $headers['X-Powered-By'] = 'meiguonet/mgboot-amphp';
        } else {
    ...
    private function handleAttachmentResponseForAmp(string $contents): array
    {
        $contents = str_replace('@attachment:', '', $contents);
        list($attachmentFilename, $contents) = explode('^^^', $contents);
    ...
    private function handleImageResponseForAmp(string $contents): array
    {
        $contents = str_replace('@image:', '', $contents);
